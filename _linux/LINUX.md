...
- 首先将服务器系统版本的ISO镜像下载到高配U盘 , 使用Rufus软件制作的USB的启动引导盘安装 (安装完成重启系统前先拔出U盘, 如不能启动再物理重启进入系统) , Help选项可提前进入shell控制
- 插入U盘启动主机大部分长按Delete或F2可以进入USB系统启动引导盘安装 (BIOS界面不同电脑快捷键不一样 有EFI源文件安装 一般台式机开机按delete键可以进入BIOS，笔记本按F2键进入BIOS)

...
class Git implements Serializable {
    ...
    static def getGitLogByTime(ctx) {
        def gitLogs = ""
        try {
            // 如 git log --pretty=format:"%s @%an %cr (%H) \n " -n 10 --since='2023-03-28 15:55:00' --no-merges
            ...
            gitLogs = Utils.getShEchoResult(ctx, "git log --pretty=format:\"%s @%an \n \" -n 100  --since='${lsbTime}' --no-merges")
            return gitLogs
        } catch (error) {

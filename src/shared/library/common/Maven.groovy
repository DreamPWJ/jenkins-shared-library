...
class Maven implements Serializable {
    ...
    static def packageBySettingFile(ctx) {
        ...
        ctx.sh "${ctx.mavenCommandType} clean install -T 2C -s ${settingsFile} -pl ${ctx.MAVEN_ONE_LEVEL}${ctx.PROJECT_NAME} -am  -Dmaven.compile.fork=true  ${ctx.isMavenTest} ${ctx.springNativeBuildParams}"
    }

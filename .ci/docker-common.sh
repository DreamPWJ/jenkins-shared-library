...
function exist_port() {
  ...
  if [[ ${port_exist} ]]; then
    ...
    get_public_ip
    public_ip=$?
    echo -e "\033[31m${public_ip}应用服务端口$1已存在, Shell自动化脚本退出运行, 端口冲突停止部署 ❌ \033[0m "
...
function get_cpu_rate() {
  ...
  if [[ ${cpu_rate} -ge 60 ]]; then
    echo " 🚨 ${public_ip}应用部署服务器CPU占用率高达: ${cpu_rate}% , 避免并发部署等导致资源阻塞, 排队延迟部署中, 等待10秒中... ☕ (如果CPU长时间占用高, 请去服务器查看资源占用情况)"

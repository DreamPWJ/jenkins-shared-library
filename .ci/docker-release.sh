...
if [[ ${is_push_docker_repo} == false ]]; then
  ...
  # 对于简单项目无需重复构建镜像  将部署文件 docker run -v 做挂载映射 直接重启容器即可
    ...
    --build-arg TOMCAT_VERSION=${tomcat_version} --build-arg JAVA_OPTS="-Xms512m -XX:MaxMetaspaceSize=512m ${docker_java_opts}" \
...
docker run -d --restart=always -p ${host_port}:${expose_port} --privileged=true --pid=host \
  ...
  -e "JAVA_OPTS=-Xms512m -XX:MaxMetaspaceSize=512m ${docker_java_opts}" -m ${docker_memory} --log-opt ${docker_log_opts} --log-opt max-file=1  ${dynamic_run_args} \

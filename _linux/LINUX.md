...
- 首先将服务器系统版本的ISO镜像下载到高配U盘 , 使用Ventoy或Rufus或balenaEtcher软件(引导ISO镜像Windows、Linux、VMware ESXI、UEFI等)制作的USB的系统启动引导盘 (安装完成reboot now重启系统前先拔出U盘, 再物理重启进入系统) , Help选项可提前进入shell控制
- 浏览器访问服务器IPMI地址管理或插入U盘启动主机大部分长按Delete或F2或F11等可以进入BIOS菜单引导页面选择USB系统启动引导盘 F10保存设置 (BIOS或新UEFI界面不同电脑快捷键不一样  一般台式机开机按delete键可以进入BIOS，笔记本按F2键进入BIOS)
- 断电后上电自动启动系统: 使用BMC页面设置或在服务器的硬件层BIOS/UEFI设置中，找到相关的自启动选项。 通常选项称为“Power AC”或类似的名称。 将其设置为“On”或“Always On”，服务器在断电后上电后主板会触发开机动作

...
(async () => {
    ...
    await page.waitForTimeout(3000) // 等待多少秒 确保页面图片加载完成
    // 位置截图
    ...
    // 自动等待 元素可见性 ，进行断言
    // const isVisibleVersion = await page.isVisible('text=版本管理');
    // expect(isVisibleVersion).toBeTruthy();
    ...
    // 审核需要填写的数据
    await page1.fill("textarea[placeholder=\"请列点简要描述本次更新的功能点\"]", "1. 优化了一些细节体验\n" +
        "2. 修复了一些已知问题")
    await page1.fill("input[placeholder=\"请提供审核测试使用的测试账号\"]", "18863302302")
    await page1.fill("input[placeholder=\"请填写测试账号的密码\"]", "123456")

    await page1.waitForTimeout(3000) // 等待多少秒

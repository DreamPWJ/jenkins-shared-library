...
class Maven implements Serializable {
    ...
    static def packageBySettingFile(ctx) {
        ...
        ctx.sh "mvn -s ${settingsFile} -pl ${ctx.MAVEN_ONE_LEVEL}${ctx.PROJECT_NAME} -am -T 1C -Dmaven.compile.fork=true clean install -Dmaven.test.skip=true ${ctx.springNativeBuildParams}"
    }

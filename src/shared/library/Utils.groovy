        ...
        try {
            ...
            if ("${ctx.PROJECT_TYPE}".toInteger() == GlobalVars.backEnd || "${ctx.PROJECT_TYPE}".toInteger() == GlobalVars.frontEnd) {
                ctx.timeout(time: 1, unit: 'MINUTES') {
                    ...
                    ctx.sh("git fetch --tags --force || true") // 更新远程所有分支tag标签  先更新标签 后按照标签时间和版本号排序
                    def versionNumArray = getShEchoResult(ctx, "git tag -l --sort=taggerdate  | sort -V", false).toString().split(" ") as ArrayList

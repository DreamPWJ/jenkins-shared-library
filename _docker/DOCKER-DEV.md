...
##### VLLM高性能部署大模型
docker run -d --restart=always -p 8008:8000 --name deepseek-vllm --gpus all --shm-size=1g -v /my/deepseek/cache:/root/.cache/huggingface \
vllm/vllm-openai:latest --model deepseek-ai/DeepSeek-R1-Distill-Qwen-14B --trust-remote-code --host 0.0.0.0 --port 8000  --dtype half --max-model-len 8192
##### Ollama只有CPU模式部署
